  ...
  const handleCompleteOnboarding = async () => {
    ...
    try {
      ...
      if (feedsToAdd.length > 0) {
        const { error: feedError } = await supabase.from('feeds').insert(feedsToAdd);
          ...
          {step === 1 && (
            ...
            >
              <div className="space-y-4 text-center">
                ...
                <h2 className="text-5xl font-serif font-medium text-gray-900 leading-tight">
                  What defines your <br /><span className="italic">curiosity?</span>
                </h2>
                <p className="text-gray-400 font-serif italic text-lg">Select the themes you wish to cultivate.</p>
              </div>
              ...
              <div className="flex flex-wrap justify-center gap-3">
                {topics.map(topic => (
                  <button
                    ...
                    className={`px-8 py-4 border transition-all duration-500 text-[11px] uppercase tracking-[0.3em] font-bold ${
              ...
              <div className="flex justify-center pt-8">
                <Button 
                  ...
                  className="rounded-none bg-white text-gray-900 border border-gray-100 hover:border-gray-900 hover:bg-white h-16 px-12 text-[10px] uppercase tracking-[0.4em] font-bold"
                >
          ...
          {step === 2 && (
            ...
            >
              <div className="space-y-4 text-center">
                ...
                <p className="text-gray-400 font-serif italic text-lg">We've gathered some notable starting points based on your interests.</p>
              </div>
                ...
                {suggestedFeeds.map(feed => (
                  ...
                  >
                    <div className="text-left">
                      <p className="font-serif font-medium text-lg text-gray-900">{feed.title}</p>
                      <p className="text-[9px] uppercase tracking-widest text-gray-400 truncate w-40">{feed.url}</p>
                    </div>
                    {selectedFeeds.includes(feed.url) && <Check size={16} className="text-gray-900" />}
                  </button>
                ))}
              </div>

              <div className="space-y-4">
                <p className="text-[10px] uppercase tracking-[0.2em] font-bold text-gray-400 text-center">Or add a custom source</p>
                <form onSubmit={handleAddCustom} className="flex gap-2">
                  <div className="relative flex-1">
                    <Globe className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300" size={14} />
                    <Input 
                      placeholder="https://your-favorite-substack.com/feed" 
                      value={customUrl}
                      onChange={(e) => setCustomUrl(e.target.value)}
                      className="h-14 pl-12 rounded-none border-gray-100 focus:border-gray-900"
                    />
                  </div>
                  <Button type="submit" disabled={loading} variant="outline" className="h-14 rounded-none px-8 border-gray-100 hover:border-gray-900">
                    <Plus size={16} />
                  </Button>
                </form>
              </div>
              ...
              <div className="flex justify-between items-center pt-8">
                ...
                <Button 
                  ...
                  disabled={loading}
                  className="rounded-none bg-gray-900 text-white h-16 px-12 text-[10px] uppercase tracking-[0.4em] font-bold"
                >
